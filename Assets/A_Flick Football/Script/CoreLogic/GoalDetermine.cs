...
public class GoalDetermine : MonoBehaviour {
	...
	public GameObject _prefabGoalAnimation;
	...
	void Awake() {
		...
		_prefabGoalAnimation.SetActive(false);
	...
	void Update () {
		...
		if(_goalCheck) {
			...
			if( _posCur.z >= _pointDown.position.z) {       // ko check goal nua
	...
	private void setGoalSuccesIcon(Area area, Vector3 position) {
		...
		_prefabGoalAnimation.SetActive(true);
